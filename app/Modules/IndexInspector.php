...
final class IndexInspector
{
    ...
    // -------------------------------------------------------------
    // Demo Mode — realistic sample data for local testing / screenshots
    // -------------------------------------------------------------

    /**
     * Detect whether Demo Mode is currently active.
     * Demo mode is honored when GSC is not actually connected — we never
     * overwrite real data on a connected site.
     */
    public function demo_mode_active(): bool
    {
        if ((int) $this->settings->get('demo_mode', 0) !== 1) {
            return false;
        }
        // If a real GSC token is present, prefer real data.
        if (!empty($this->settings->get_encrypted('gsc_access_token'))) {
            return false;
        }
        return true;
    }
    ...
    /**
     * Generate a realistic demo inventory based on the site's actual posts/pages.
     * Deterministic per-post (uses post_id as seed) so the demo doesn't reshuffle
     * on every page refresh.
     */
    public function demo_list_for_site(int $site_id): array
    {
        $posts = get_posts([
            'post_type'      => ['post', 'page'],
            'post_status'    => 'publish',
        ...
        foreach ($posts as $i => $post) {
            $items[] = $this->demo_row_for_post($post, $i);
        }
        return $items;
    }

    public function demo_summary(int $site_id): array
    {
        $rows = $this->demo_list_for_site($site_id);
        $total = count($rows);
        $indexed = 0; $crawled_not = 0; $discovered_not = 0; $excluded = 0; $high_risk = 0;
        foreach ($rows as $r) {
            $state = (string) ($r['coverage_state'] ?? '');
            if (stripos($state, 'submitted and indexed') !== false) {
                $indexed++;
            } elseif (stripos($state, 'crawled') !== false) {
                $crawled_not++;
            } elseif (stripos($state, 'discovered') !== false) {
                $discovered_not++;
            } elseif (stripos($state, 'excluded') !== false) {
                $excluded++;
            }
            if ((int) $r['risk_score'] >= 70) {
                $high_risk++;
            }
        }
        return [
    ...
    public function demo_detect_patterns(int $site_id): array
    {
        $rows = $this->demo_list_for_site($site_id);
        $rejected = array_filter($rows, fn ($r) => !$r['is_indexed']);
        $total_rejected = count($rejected);
        if ($total_rejected < 2) {
            return [];
        }
        ...
        $thin = count(array_filter($rejected, fn ($r) => !empty($r['signals']['thin_content'])));
        if ($thin >= 2) {
            $patterns[] = [
                'key'     => 'thin_content',
                'count'   => $thin,
        ...
        $orphans = count(array_filter($rejected, fn ($r) => !empty($r['signals']['is_orphan'])));
        if ($orphans >= 2) {
            $patterns[] = [
                'key'     => 'orphans',
                'count'   => $orphans,
                ...
                'message' => sprintf(
                    __('%1$d of %2$d rejected pages have no incoming internal links. Google deprioritises orphans.', 'nexora-pulse'),
                    $orphans, $total_rejected
                ),
                'fix'     => __('Add 2–5 internal links from related, high-authority pages to each affected URL.', 'nexora-pulse'),
            ];
        }

        $dupes = count(array_filter($rejected, fn ($r) => !empty($r['signals']['near_duplicate'])));
        if ($dupes >= 2) {
            $patterns[] = [
                'key'     => 'duplicates',
                'count'   => $dupes,
    ...
    private function demo_row_for_post(WP_Post $post, int $idx): array
    {
        // Deterministic "fate" based on post ID — same post always lands in
        // the same demo state across refreshes.
        $fate = (int) $post->ID % 7;
        $url  = (string) get_permalink($post->ID);

        // Distribute states roughly: ~45% indexed, ~25% crawled-not, ~15% discovered-not, ~15% excluded.
        switch ($fate) {
            case 0: case 1: case 2:
                ...
                break;
            case 3:
                $state = 'Indexed, not submitted in sitemap';
                $verdict = 'PASS';
                $score = 15;
                $reasons = [[
                    'severity' => 'low',
                    'message'  => __('Page is indexed but not in your XML sitemap — add it to make sure Google revisits regularly.', 'nexora-pulse'),
                ]];
                break;
            case 4:
                ...
                $score = 78;
                $reasons = [
                    ['severity' => 'critical', 'message' => __('Google crawled this page but decided NOT to index it. This usually indicates a quality or duplication signal.', 'nexora-pulse')],
                    ['severity' => 'high',     'message' => __('Only 240 words on this page. Thin content rarely earns a Google index slot.', 'nexora-pulse')],
                    ['severity' => 'medium',   'message' => __('No internal links point to this page. Add links from related high-authority content.', 'nexora-pulse')],
                ];
                break;
            case 5:
                ...
                $score = 55;
                $reasons = [
                    ['severity' => 'high',   'message' => __('Google knows this URL exists but hasn\'t crawled it. Often a crawl-budget or authority signal.', 'nexora-pulse')],
                    ['severity' => 'medium', 'message' => __('Page age 14 days — give Google a couple more weeks, or submit via the Indexing API.', 'nexora-pulse')],
                ];
                break;
            default:
                $state = 'Excluded by \'noindex\' tag';
                $verdict = 'FAIL';
                $score = 88;
                $reasons = [[
                    'severity' => 'critical',
                    'message'  => __('Page is explicitly excluded from indexing (noindex directive). Remove it if this page should rank in search.', 'nexora-pulse'),
                ]];
        }

        // Signals — vary realistically so pattern detector finds something interesting.
        $signals = [
            'word_count'     => 240 + (($post->ID * 17) % 800),
            'thin_content'   => in_array($fate, [4], true),
            'incoming_links' => max(0, 3 - $fate),
            'is_orphan'      => in_array($fate, [4, 5], true),
            'near_duplicate' => $fate === 4,
            'age_days'       => 14 + (($post->ID * 3) % 400),
            'stale'          => false,
            'has_meta_desc'  => $fate !== 4,
            'is_noindex'     => $fate === 6,
        ];
        ...
        return [
            ...
            'indexing_state'    => $fate <= 3 ? 'INDEXING_ALLOWED' : 'INDEXING_BLOCKED_BY_META_TAG',
            'page_fetch_state'  => 'SUCCESSFUL',
            'last_crawl_time'   => gmdate('Y-m-d H:i:s', time() - (($post->ID * 3600) % (14 * DAY_IN_SECONDS))),
            ...
            'is_indexed'        => in_array($fate, [0, 1, 2, 3], true),
        ];
    }
